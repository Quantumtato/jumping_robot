...
#include "mc_config.h"
#include "mc_type.h"
#include "pmsm_motor_parameters.h"  /* POLE_PAIR_NUM, MOTOR_VOLTAGE_CONSTANT from MC Workbench */
...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define MY_MOTOR_ID       0x01
#define MOTOR_KT          0.0095f          // [Nm/A] - measured torque constant (not stored by Workbench)
#define MOTOR_POLE_PAIRS  POLE_PAIR_NUM    // [Pole Pairs] - pulled from pmsm_motor_parameters.h
#define IQMAX_A           45.0f       // [A]  - must match IQMAX_A in drive_parameters.h
...
// Hardcoded Motor ID for this specific board configuration
#define MY_MOTOR_ID   0x01
#define CTRL_ID       (MY_MOTOR_ID + 0x200)  // 0x201 — state machine commands from GUI
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs)
{
    ...
    if ((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != RESET)
    {
        ...
        if (HAL_FDCAN_GetRxMessage(hfdcan, FDCAN_RX_FIFO0, &rxHeader, rxData) == HAL_OK)
        {
            ...
            else if (rxHeader.Identifier == CTRL_ID && rxHeader.RxFrameType == FDCAN_DATA_FRAME
