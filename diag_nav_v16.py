...
CHECKPOINT = (
    ...
    "2026-08-20_04-27-37_direct_continue_v16/model_5999.pt"
)
NUM_ENVS = 16
BLOCK_STEPS = 700
CMD_SPEED = 0.12
...
for name, cmd in BLOCKS.items():
    ...
    # ---- fall classification: what phase preceded each reset? ----
    arr = {k: np.stack(v) for k, v in log.items()}  # (T, N, ...)
    fall_phase = {"flight": 0, "landing": 0, "stance": 0, "timeout": 0}
    for e in range(NUM_ENVS):
        last_touchdown = -999
        for t in range(1, BLOCK_STEPS):
            if arr["contact"][t, e] and not arr["contact"][t - 1, e]:
                last_touchdown = t
            if arr["reset"][t, e] and not arr["reset"][t - 1, e]:
                pre = t - 1
                if pre - 0 > 1900:  # ~20 s episode: likely timeout
                    fall_phase["timeout"] += 1
                elif not arr["contact"][pre, e]:
                    fall_phase["flight"] += 1
                elif pre - last_touchdown <= 50:
                    fall_phase["landing"] += 1
                else:
                    fall_phase["stance"] += 1
    hops = []
    for e in range(NUM_ENVS):
        ...
        while t < BLOCK_STEPS:
            ...
            t += 1
    results[name] = hops
    print(f"[block {name}] resets by phase: {fall_phase}")
